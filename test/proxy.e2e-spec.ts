...
const mockFirebaseAdmin = {
  auth: () => ({
    ...
    updateUser: jest.fn((uid, { email, locked }) => {
...
describe('ProxyController (e2e)', () => {
  ...
  it('PATCH /admin-backend/users/:uuid/lock-status should update the user\'s lock status', async () => {
    const uid = "test-uid";
    const res = await request(app.getHttpServer())
  ...
  it('PATCH /admin-backend/users/:uuid/lock-status should fail if an invalid token is passed', async () => {
    const res = await request(app.getHttpServer())
      .patch(`/admin-backend/users/uid/lock-status`)
      .set('Authorization', 'Bearer invalid-token')
      .send({ locked: true });
