...
import { startMockCoursesService } from './mock-courses.service';
...
describe('ProxyController (e2e)', () => {
    ...
    let userServer, coursesServer;

    beforeAll(async () => {
        userServer = startMockUserService(3001);
        coursesServer = startMockCoursesService(3002);
    ...
    it('GET /education/ping should proxy to courses service', async () => {
        const res = await request(app.getHttpServer())
            .get('/courses/ping')
            .set('Authorization', 'Bearer mock-token');
